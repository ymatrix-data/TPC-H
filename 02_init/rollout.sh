...
check_gucs()
{
	...
	if [ "$VERSION" == "gpdb_5" ]; then
		counter=$(psql -v ON_ERROR_STOP=1 -q -AXtc "show optimizer_join_arity_for_associativity_commutativity" | grep -i "18" | wc -l; exit ${PIPESTATUS[0]})
	...
	counter=$(psql -v ON_ERROR_STOP=1 -q -AXtc "show optimizer_analyze_root_partition" | grep -i "on" | wc -l; exit ${PIPESTATUS[0]})
	if [ "$counter" -eq "0" ]; then
	...
	counter=$(psql -v ON_ERROR_STOP=1 -q -AXtc "show gp_autostats_mode" | grep -i "none" | wc -l; exit ${PIPESTATUS[0]})
	if [ "$counter" -eq "0" ]; then
	...
	counter=$(psql -v ON_ERROR_STOP=1 -q -AXtc "show default_statistics_target" | grep "100" | wc -l; exit ${PIPESTATUS[0]})
	if [ "$counter" -eq "0" ]; then
