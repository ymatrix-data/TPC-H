...
for i in $(ls $sql_file_path/*.$filter.*.sql); do
  ...
  CREATE_MARS_BTREE_INDEX=""
	...
	echo "psql -v ON_ERROR_STOP=1 -q -a -P pager=off -f $i -v SMALL_STORAGE=\"$SMALL_STORAGE\" -v MEDIUM_STORAGE=\"$MEDIUM_STORAGE\" -v LARGE_STORAGE=\"$LARGE_STORAGE\" -v DISTRIBUTED_BY=\"$DISTRIBUTED_BY\" -v CREATE_MARS3_BTREE_INDEX=\"$CREATE_MARS_BTREE_INDEX\" -v CREATE_EXTENSION=\"$CREATE_EXTENSION\""
